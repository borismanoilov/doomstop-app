        ...
        ZStack(alignment: .bottom) {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    Text("Confirm blocked apps")
                        .font(.custom(Theme.Fonts.headlineSemiBold, size: 30))
                        .foregroundColor(Color(hex: "#1C1C1C"))
                        .padding(.bottom, 6)
                        .opacity(showHeadline ? 1 : 0)
                        .offset(y: showHeadline ? 0 : 14)

                    Text("These are the apps Doomstop will intercept. Change anytime in Settings.")
            ...
            VStack(spacing: 0) {
                ...
                Button(action: {
                    appState.blockedApps = selected
                    navigationRouter.currentPhase = .intention
                }) {
                    Text(selected.isEmpty ? "Select apps" : "Confirm \(selected.count) app\(selected.count == 1 ? "" : "s") →")
...
#Preview {
    AppSelectView(initialApps: ["TikTok", "Instagram"])
